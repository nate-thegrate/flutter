...
class _SnippetChecker {
      ...
      try {
        ...
        for (final String line in fileLines) {
          ...
          if (inExamplesCanAssumePreamble) {
            ...
            if (_dartDocSnippetBeginRegex.firstMatch(trimmedLine) != null) {
  ...
  List<Object> _analyze(Map<String, _SnippetFile> snippets) {
    ...
    for (final String error in analyzerOutput) {
      ...
      final int snippetLength = snippet.code.length;
      if (fileContents.length != snippetLength) {
        errors.add(_SnippetCheckerException(
          'Unexpected file contents for ${file.path}. File has ${fileContents.length} lines '
          'but we generated $snippetLength lines:\n${snippet.code.join("\n")}',
          file: file.path,
          line: lineNumber,
        ));
        continue;
      ...
      for (int delta = 0; errorData == null; delta += 1) {
        ...
        assert(before >= 0 || after < snippetLength);
        if (before >= 0 && before < snippetLength && !snippet.code[before].generated) {
          source = snippet.code[before];
          errorData = (
            line: source.line,
            column: delta == 0 ? columnNumber : source.code.length + 1,
            message: delta == 0 ? message : '$message -- in later generated code',
          );
        } else if (after >= 0 && after < snippetLength && !snippet.code[after].generated) {
          source = snippet.code[after];
          errorData = (
            line: source.line,
            column: 1,
