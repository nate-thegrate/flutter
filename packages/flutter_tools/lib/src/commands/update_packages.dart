...
class UpdatePackagesCommand extends FlutterCommand {
  UpdatePackagesCommand({required bool verboseHelp}) {
    argParser
      ...
      ..addOption(
        _keyVersion,
        help:
            'A Flutter version to use in place of the version determined from git tags. '
            'This is useful in environments where "git fetch" cannot be used to retrieve '
            'upstream tags (for example, to satisfy Flutter SDK version constraints in '
            'pubspecs during "flutter update-packages").\n'
            'Example: flutter update-packages --flutter-version 3.45.0',
      );
  }
  ...
  final _keyVersion = 'flutter-version';
  ...
  Future<FlutterCommandResult> runCommand() async {
    ...
    final String? explicitVersion = stringArg(_keyVersion);
    if (explicitVersion != null) {
      _setFlutterVersion(explicitVersion);
    }
  ...
  void _setFlutterVersion(String version) {
    globals.printStatus('Using explicit Flutter version $version.');
    final String flutterRoot = Cache.flutterRoot!;
    final File versionFile = FlutterVersion.getVersionFile(globals.fs, flutterRoot);
    var revision = '0000000000000000000000000000000000000000';
    try {
      revision = globals.git
          .logSync(['-n', '1', '--pretty=format:%H'], workingDirectory: flutterRoot)
          .stdout
          .trim();
    } on Object catch (_) {
      // Ignore; fallback revision is already set.
    }
    if (revision.isEmpty) {
      revision = '0000000000000000000000000000000000000000';
    }
    final String engineRevision = globals.cache.engineRevision;
    final String dartSdkVersion = globals.cache.dartSdkVersion;
    final String devToolsVersion = globals.cache.devToolsVersion;
    ...
    final Map<String, Object> manifest = <String, Object>{
      ...
      'frameworkCommitDate': now,
      'engineRevision': engineRevision,
      'engineCommitDate': now,
    ...
    versionFile.writeAsStringSync(encoder.convert(manifest));
    // Also write legacy version file for compatibility.
    final File legacyVersionFile = globals.fs.file(
      globals.fs.path.join(flutterRoot, 'version'),
    );
    legacyVersionFile.writeAsStringSync(version);
  }
