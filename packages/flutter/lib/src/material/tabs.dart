...
class Tab extends StatelessWidget implements PreferredSizeWidget {
  ...
  Size get preferredSize {
    ...
    } else if ((text ?? child) != null && icon != null) {
...
class _TabBarState extends State<TabBar> {
  ...
  void _updateTabController() {
    ...
    if (_controller case final TabController controller) {
      controller.animation!.addListener(_handleTabControllerAnimationTick);
  ...
  Widget build(BuildContext context) {
    ...
    if (_controller case final TabController controller) {
      ...
      if (controller.indexIsChanging) {
        ...
        final Animation<double> centerAnimation = _DragAnimation(controller, tabIndex);
        wrappedTabs[tabIndex] = _buildStyledTab(wrappedTabs[tabIndex], true, centerAnimation, _defaults);
        if (_currentIndex! > 0) {
          final int tabIndex = _currentIndex! - 1;
          final Animation<double> previousAnimation = ReverseAnimation(_DragAnimation(controller, tabIndex));
          wrappedTabs[tabIndex] = _buildStyledTab(wrappedTabs[tabIndex], false, previousAnimation, _defaults);
        }
        if (_currentIndex! < widget.tabs.length - 1) {
          final int tabIndex = _currentIndex! + 1;
          final Animation<double> nextAnimation = ReverseAnimation(_DragAnimation(controller, tabIndex));
